...
for q=1:Q
    ...
    for m=1:M-1
        Hy(m) = Hy(m) + ( (deltaT/deltaX)/(imp0*mu_r) ) *...
            ( Ezn(m+1) - Ezn(m) );
    end
    %% Actualizacion del campo electrico
     for m=2:M
        Ezn(m) = Ezn(m) + ((deltaT/deltaX)/eps_r(m))*...
            ( Hy(m) - Hy(m-1) );
    end
%     for m=2:M
%         DznMas1(m) = Dzn(m) + (deltaT/deltaX)*...
%             ( Hy(m) - Hy(m-1) );
%     end
    %% Actualización del campo eléctrico
%     for m=2:M
%     EznMas1=(((w0^2)*(deltaT^2) + 2*delta*deltaT + 2)*DznMas1-4*Dzn +...
%         ((w0^2)*(deltaT^2) - 2*delta*deltaT + 2)*DznMenos1 + 4*eps0*Ezn -...
%         ((w0^2)*(deltaT^2)*eps_s - 2*delta*deltaT*eps0 + 2*eps0).*EznMenos1) ./...
